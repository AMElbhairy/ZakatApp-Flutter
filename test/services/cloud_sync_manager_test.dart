...
void main() {
  ...
  test(
    'CloudSyncManager CheckForUpdates returns noRemoteSnapshot when cloud is empty',
    () async {
  ...
  test(
    'CloudSyncManager pushes, checklist history, and prunes older than 5 snapshots',
    () async {
      // Setup original DB
      ...
      for (int i = 1; i <= 6; i++) {
        ...
        final pushResult = await syncManagerDeviceA.pushSnapshot(
          db: db,
          customDbPath: dbPath,
        );
      ...
      expect(
        manifest.snapshots.map((s) => s.sequence).toList(),
        equals([2, 3, 4, 5, 6]),
      );
      ...
      expect(
        cloudFiles.any((f) => f.path.contains('snapshot_00000001.sqlite.enc')),
        isFalse,
      );
      expect(
        cloudFiles.any((f) => f.path.contains('snapshot_00000006.sqlite.enc')),
        isTrue,
      );
    },
  );
  ...
  test(
    'CloudSyncManager CheckForUpdates detects up-to-date and newer remote snapshots',
    () async {
      ...
      final dbA = AppDatabase(
        userId: 'user-a',
        executor: NativeDatabase(dbFileA),
      );
      ...
      final pullResult = await syncManagerDeviceB.pullAndRestore(
        targetPath: dbPathB,
      );
      ...
      final localChecksumB = await syncManagerDeviceB.calculateFileChecksum(
        dbPathB,
      );
  ...
  test(
    'Manifest update with stale revision fails and leaves snapshot and manifest in recoverable state',
    () async {
      // 1. Setup DB
      final dbPath = p.join(tempDir.path, 'db_opt.sqlite');
      final db = AppDatabase(
        userId: 'user-opt',
        executor: NativeDatabase(File(dbPath)),
      );
      await db.customStatement(
      ...
      final pushResult1 = await syncManagerDeviceA.pushSnapshot(
        db: db,
        customDbPath: dbPath,
      );
      ...
      await db.customStatement(
        "UPDATE app_settings SET value_json = '\"red\"' WHERE key = 'color'",
      );
      final pushResult2 = await syncManagerDeviceA.pushSnapshot(
        db: db,
        customDbPath: dbPath,
      );
      ...
      final manifestFinal = CloudSyncManifest.fromJson(
        manifestMetaFinal.content,
      );
  ...
  test(
    'replaceActiveDatabaseWithRestoredFile closes active DB, creates timestamped backup, overwrites with restored DB, reopens, and reloads state',
    () async {
      ...
      final activeCheck = await controller.database!
      ...
      final tempRestoredDb = AppDatabase(
        userId: 'restored-source',
        executor: NativeDatabase(File(tempRestoredPath)),
      );
      await tempRestoredDb.customStatement(
        "INSERT INTO app_settings (key, value_json, updated_at) VALUES ('zakat_paid_months', '[\"2026-06\"]', '2026-06-23T08:00:00Z')",
      );
      await tempRestoredDb.customStatement(
        "INSERT INTO app_settings (key, value_json, updated_at) VALUES ('zakat_expense_ids', '{\"2026-06\":\"tx-zakat\"}', '2026-06-23T08:00:00Z')",
      );
      await tempRestoredDb.customStatement(
        "INSERT INTO app_settings (key, value_json, updated_at) VALUES ('processed_expense_ids', '[\"tx-zakat\"]', '2026-06-23T08:00:00Z')",
      );
      ...
      expect(controller.state.zakatPaidMonths, equals(<String>['2026-06']));
      expect(
        controller.state.zakatExpenseIds,
        equals(<String, dynamic>{'2026-06': 'tx-zakat'}),
      );
      expect(
        controller.state.processedExpenseIds,
        equals(<String>['tx-zakat']),
      );

      final verifyCheck = await controller.database!
          .customSelect("SELECT * FROM transactions")
          .get();
      ...
      final backupFiles = dirFiles
          .where(
            (f) =>
                f.path.contains('restore_backup_') && f.path.endsWith('.bak'),
          )
          .toList();
      expect(backupFiles.length, equals(1));

      final backupFile = File(backupFiles.first.path);
      final backupDb = AppDatabase(
        userId: 'backup-verify',
        executor: NativeDatabase(backupFile),
      );
      final backupCheck = await backupDb
          .customSelect("SELECT * FROM transactions")
          .get();
