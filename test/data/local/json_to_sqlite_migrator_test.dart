...
void main() {
  ...
  test(
    'imports transactions savings pending and investments and enqueues sync operations',
    skip: true,
    () async {
      ...
      final pendingRows = await database
          .select(database.pendingTransactions)
          .get();
  ...
  test('migration imports zakat settings into SQLite settings rows', () async {
    ...
    expect(settings['zakat_expense_ids'], <String, dynamic>{
      '2026-06': 'tx-paid',
    });
  ...
  test(
    'migration preserves language preference in SQLite app settings',
    () async {
      final repository = _CountingRepository(
        AppStateModel.fromJson(<String, dynamic>{'languagePreference': 'ar'}),
      );
      final migrator = JsonToSqliteMigrator(
        database: database,
        migrationStateDao: migrationStateDao,
      ...
      expect(result, isTrue);
      expect(settings['language_preference'], 'ar');
    },
  );

  test(
    'migration imports financial plans into SQLite financial plans table',
    () async {
      final repository = _CountingRepository(
        AppStateModel.fromJson(<String, dynamic>{
  ...
  test(
    'imports merchant confirmations into SQLite merchant confirmations table',
    () async {
      final repository = _CountingRepository(
        AppStateModel.fromJson(<String, dynamic>{
  ...
  test(
    'imports correction feedback into SQLite correction feedback table',
    () async {
      final repository = _CountingRepository(
        AppStateModel.fromJson(<String, dynamic>{
