...
- (UIViewController *)viewControllerWithWindow:(UIWindow *)window {
    ...
    if (windowToUse == nil) {
        if (@available(iOS 13.0, *)) {
            ...
            for (UIWindow *sceneWindow in [UIApplication sharedApplication].windows) {
                if (sceneWindow.isKeyWindow) {
                    windowToUse = sceneWindow;
                    break;
                }
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if([call.method isEqualToString:@"clear"]) {
        _result([NSNumber numberWithBool: [ZWFileUtils clearTemporaryFiles]]);
        _result = nil;
        return;
    }
    ...
    if([call.method isEqualToString:@"any"] || [call.method containsString:@"custom"]) {
        self.allowedExtensions = [ZWFileUtils resolveType:call.method withAllowedExtensions: [arguments valueForKey:@"allowedExtensions"]];
...
#ifdef PICKER_MEDIA
        [self resolvePickMedia:[ZWFileUtils resolveMediaType:call.method] withMultiPick:isMultiplePick withCompressionAllowed:self.allowCompression];
#else
        _result([FlutterError errorWithCode:@"Unsupported picker type"
...
- (void) handleResult:(id) files {
    _result([ZWFileUtils resolveFileInfo: [files isKindOfClass: [NSArray class]] ? files : @[files] withData:self.loadDataToMemory]);
    _result = nil;
}
...
- (void)mediaPicker: (MPMediaPickerController *)mediaPicker didPickMediaItems:(MPMediaItemCollection *)mediaItemCollection
{
    ...
    for(MPMediaItemCollection * item in [mediaItemCollection items]) {
        NSURL * cachedAsset = [ZWFileUtils exportMusicAsset: [item valueForKey:MPMediaItemPropertyAssetURL] withName: [item valueForKey:MPMediaItemPropertyTitle]];
